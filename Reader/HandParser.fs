...
let (|NewSeq|_|) toks = match toks with | New::Format(ft)::Limit(lt)::Const(ante)::Const(sb)::Const(bb)::t -> Some(ft, lt, ante, sb, bb) | _ -> None
...
let (|AnteSeq|_|) toks = match toks with | Ante::Const(ante)::t -> Some(ante) | _ -> None
...
let rec parse (toks: Token list) (gs: GameState) (h: Hand list) = 
    match toks with 
    | [] -> h
    | NewSeq(ft, lt, ante, sb, bb) -> parseNewPlayers (toks |> List.skip 6) gs h ft lt ante sb bb [] Map.empty
    ...
    | BlindsSeq(sb, bb) -> parse (toks |> List.skip 3) { gs with Blinds = Types.Blinds (sb, bb) } h
    | AnteSeq(ante) -> parse (toks |> List.skip 2) { gs with Ante = ante } h
    | _ -> failwithf "Unexpected token %A in parse environment" toks.Head
and parseNew toks gs h (ft: FormatType) (lt: LimitType) (ante: decimal) (sb: decimal) (bb: decimal) (ps: string list) (ss: Map<string, decimal>) = 
    parse toks { GameType = (ft, lt); Ante=ante; Blinds=Types.Blinds(sb, bb); Players=ps; Stacks=ss } h
and parseNewPlayers toks gs h ft lt ante sb bb ps ss = 
    match toks with
    | (Name(p))::(Const(f))::t -> parseNewPlayers t gs h ft lt ante sb bb (p::ps) (Map.add p f ss)
    | _ -> parseNew toks gs h ft lt ante sb bb (List.rev ps) ss
